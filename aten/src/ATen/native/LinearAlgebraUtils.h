...
inline bool can_flatten_batch_dims(const Tensor& A) {
  ...
  for (int i = A.dim() - 3; i >= 0; --i) {
    if (strides[i] != right_stride * sizes[i + 1]) {
      return false;
    } else {
      right_stride = strides[i];
    }
  }
...
/*
 * Check whether a (batched) matrix is compliant with being
 * a col-major-like, i.e. contiguous cols with a stride between them at least the number of rows.
 * NOTE: rows/cols overlaps is a disqualifying property!
 */
inline bool isColMajorLike(const Tensor& t) {
  TORCH_INTERNAL_ASSERT_DEBUG_ONLY(t.dim() >= 2);
  return is_non_overlapping_matrices(t) && ld_complies(t, /*check_col_major_like*/true);
}
...
inline bool isRowMajorLike(const Tensor& t) {
  TORCH_INTERNAL_ASSERT_DEBUG_ONLY(t.dim() >= 2);
  return is_non_overlapping_matrices(t) && ld_complies(t, /*check_col_major_like*/false);
}
