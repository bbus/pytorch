...
cusparseDnMatDescr_t createRawDnMatDescriptor(const Tensor& input, int64_t batch_offset, bool is_const=false) {
  ...
  TORCH_INTERNAL_ASSERT(
      at::native::can_flatten_batch_dims(input) && (is_column_major_like || is_row_major_like),
      "Expected either row or column major input with a uniform across batch dims stride.");
